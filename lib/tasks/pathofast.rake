namespace :pathofast do
  desc "TODO"
    ...
    task prepare_ruby_astm_env: :environment do 
        ...
        item_type_object = Inventory::ItemType.new(JSON.parse(IO.read(Rails.root.join("test","test_json_models","inventory","item_types","BD_Citrate_tube.json"))))
    ...
    task pathofast_lis_test: :environment do 
        ...
        path = Rails.root.join('test','test_json_models','inventory','item_types','**/*.json')

        Dir.glob(path).each do |file|
        
            item_type_object = JSON.parse(IO.read(file))
            item_type = nil
            ...
            item_group = Inventory::ItemGroup.new
            item_group.name = item_type.name
            item_group.group_type = "Consumables"
            item_group.item_definitions = [
                {
                    item_type_id: item_type.id.to_s,
            ...
            barcode = nil
            if item_type.name =~ /SST/i
                barcode = "000009041"
            elsif item_type.name =~ /Fluoride/i
                barcode = "0000010219"
            elsif item_type.name =~ /EDTA/i
                barcode = "000007707"
            end
            unless barcode.blank?
                ...
                unless item.errors.full_messages.blank?
                    ...
                    puts "ITEM CREATED with barcode: #{barcode}"
                end
            end
        ...
        ["biochemistry/others/glucose_random.json","fluids/urine_routine.json","hematology/ESR.json","hematology/CBC.json","immunoassay/HIV.json","immunoassay/HBV.json"].each do |report_file_name|

            path = Rails.root.join('vendor','assets','pathofast_report_formats')
            path += report_file_name

            #puts "path is: #{path}"
            #puts "the file becomes ------------------>"
            #puts IO.read(path)

            r = Diagnostics::Report.new(JSON.parse(IO.read(path)))
            r.created_by_user = user
            r.created_by_user_id = user.id.to_s
            r.save
            #report_ids << r.id.to_s
        ...
        puts "lis security key:" + organization.lis_security_key
        puts "organization id:" + organization.id.to_s
